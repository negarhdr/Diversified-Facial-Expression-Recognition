...
parser.add_argument('--data', type=str, default='../FER_data/AffectNet_224/', help='path to dataset')
parser.add_argument('--checkpoint_path', type=str, default='./checkpoint/affectNet/'+time_str+'div_manet.pth')
parser.add_argument('--best_checkpoint_path', type=str, default='./checkpoint/affectNet/'+time_str+'div_manet_best.pth')
...
class FeatDiversity(nn.Module):
    ...
    def forward(self, x):
        ...
        gamma = 10
        snm = torch.zeros((num_branches, num_branches))
        # diversity between channels of the two branch features
        for i in range(num_branches):
            for j in range(num_branches):
                if i != j:
                    diff = torch.exp(
...
def main():
    ...
    train_data = udata.AffectNetCategorical(idx_set=0,
                                            ...
                                            base_path_to_affectnet=args.data)
...
def validate(val_loader, model, criterion_ce, args):
    ...
    with torch.no_grad():
        ...
        with open('./log/' + time_str + '.txt', 'a') as f:
